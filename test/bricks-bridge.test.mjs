...
const ELEMENTS = [
  ...
  { id: 'head1', name: 'heading', parent: 'sec1', children: [], settings: { text: 'Example Drop 01', tag: 'h1' } },
...
test('imported page is NATIVE canvas: renders publicly once published', async () => {
  ...
  assert.match(res.body, /Example Drop 01/, 'heading renders');
...
test('export round-trip: canvas → Bricks JSON that Bricks would accept', async () => {
  ...
  assert.equal(heading.settings.text, 'Example Drop 01');
