...
async function save(domain: string, field: string, value: unknown): Promise<void> {
  ...
  // A rejected PATCH used to be swallowed: the toggle flipped optimistically and
  // stayed flipped while nothing saved. Throw so the control reverts and shows why.
  if (!res.ok) {
...
function SaveError({ msg }: { msg: string | null }) {
  if (!msg) return null;
  return <span className="field-help" style={{ color: 'var(--th-danger, #dc2626)' }}>{msg}</span>;
}
...
export function TextInput({
  ...
  async function commit(): Promise<void> {
    ...
    try {
      ...
      setValue(initial); // revert to last-saved
      setErr(e instanceof Error ? e.message : 'Save failed.');
    } finally {
      setBusy(false);
    }
