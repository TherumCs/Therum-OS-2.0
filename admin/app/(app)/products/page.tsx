...
import { ProductBulkTable } from './ProductBulkTable';
...
interface SP { status?: string; q?: string; sort?: string; order?: string; cursor?: string; page?: string; categoryId?: string; tagId?: string; view?: string; add?: string; trashed?: string }

export default async function ProductsPage({ searchParams }: { searchParams: Promise<SP> }) {
  const sp = await searchParams;
  const per = await perPage();
  const pageNum = sp.page && /^\d+$/.test(sp.page) ? Number(sp.page) : 1;
  const qs = new URLSearchParams({ limit: String(per) });
  if (sp.status && sp.status !== 'all') qs.set('status', sp.status);
  if (sp.q) qs.set('q', sp.q);
  if (sp.sort) qs.set('sort', sp.sort);
  if (sp.order) qs.set('order', sp.order);
  // Numbered-page offset paging (default page 1); supersedes the old cursor.
  ...
  return (
    <section>
      ...
      {data && data.items.length > 0 && (grid ? (
        ...
        // List view carries row selection + a bulk-action bar (client island).
        // Grid view above stays a plain server-rendered set of links.
        <ProductBulkTable items={data.items} trashView={trashView} />
      ))}
      {data && <ListPager nextCursor={data.nextCursor ?? null} shown={data.items.length} total={data.total ?? data.items.length} page={pageNum} perPage={per} />}
    </section>
  );
}
