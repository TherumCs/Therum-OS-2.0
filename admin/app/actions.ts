...
export interface BulkResult { ok: number; total: number; failed: { id: string; error: string }[] }

/** Apply one lifecycle action to a selection. Mirrors the single-row actions;
 *  the backend runs each id through the same service method. */
export async function bulkProducts(
  ids: string[],
  action: 'status' | 'trash' | 'restore' | 'purge' | 'assign',
  extra?: { status?: 'active' | 'draft' | 'archived'; categoryIds?: string[]; tagIds?: string[] },
): Promise<BulkResult> {
  const res = await apiSend<BulkResult>('POST', '/api/products/bulk', { ids, action, ...extra });
  revalidatePath('/products');
  return res;
}
