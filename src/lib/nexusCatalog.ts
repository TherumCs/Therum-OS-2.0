...
export const nexusCatalog: CatalogProvider[] = [
  ...
  // the merchant's named POD partners (2026-07-24): store-and-hold until their fleet
...
{ id: 'jetprint', name: 'JetPrint', category: 'fulfillment', authType: 'apikey', join: '|', issuedBy: 'your-store', connectsVia: 'store-pull-woo', credentialHint: 'Copy these three values out of JetPrint and paste them here.', fields: [{ label: 'Website' }, { label: 'Consumer key', secret: true }, { label: 'Consumer secret', secret: true }] },
  ...
  // WooPayments rides the sealed engine, not a pasted key — its "credential" is
  // just the bridge webhook secret so the gateway resolves. Connected/managed
  // from Counter › Payments, never a store key the operator types here.
  { id: 'woopay', name: 'WooPayments', category: 'payments', authType: 'apikey', issuedBy: 'provider', credentialHint: 'Managed by the payment engine — connected from Counter › Payments.', fields: [{ label: 'Bridge secret', secret: true }], note: 'Routes card / wallet / BNPL to the WooPayments merchant account (instant-payout eligible). Managed from Payments, not connected with a key here.' },
