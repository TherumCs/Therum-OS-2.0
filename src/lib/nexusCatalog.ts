...
export const nexusCatalog: CatalogProvider[] = [
  ...
  { id: 'gooten', name: 'Gooten', category: 'fulfillment', authType: 'apikey', issuedBy: 'provider', join: '|', credentialHint: 'Both are in Gooten: Settings > API', fields: [{ label: 'Recipe ID' }, { label: 'Partner Billing Key', secret: true }], connectsVia: 'store-pull-woo', note: 'No connection test yet: Gooten addresses its API by Recipe ID in the query string and the probe endpoints answered 404, so a tester would report a wrong URL as a bad key. The two fields match Gooten Settings > API.' },
  ...
  { id: 'podplus', name: 'Podplus', category: 'fulfillment', authType: 'apikey', issuedBy: 'provider', credentialHint: 'Credential shape NOT verified — no public API docs found. Confirm with the vendor before relying on this.', fields: [{ label: 'API Key', secret: true }], connectsVia: 'store-pull-woo', note: 'No connection test: no public API documentation was found. The field is a placeholder — confirm it with the vendor before relying on it.' },
  { id: 'podpartner', name: 'PodPartner', category: 'fulfillment', authType: 'apikey', issuedBy: 'provider', credentialHint: 'Credential shape NOT verified — no public API docs found. Confirm with the vendor before relying on this.', fields: [{ label: 'API Key', secret: true }], connectsVia: 'store-pull-woo', note: 'No connection test: no public API documentation was found. The field is a placeholder — confirm it with the vendor before relying on it.' },
  { id: 'tapstitch', name: 'Tapstitch', category: 'fulfillment', authType: 'apikey', join: '|', issuedBy: 'your-store', credentialHint: 'Generated by your store, then given to Tapstitch', fields: [{ label: 'Consumer Key' }, { label: 'Consumer Secret', secret: true }], connectsVia: 'store-pull-woo', note: 'No connection test: these are keys YOUR store issues for Tapstitch to read it, so there is nothing of theirs to call. Generate them with the button above.' },
  { id: 'contrado', name: 'Contrado', category: 'fulfillment', authType: 'apikey', issuedBy: 'provider', credentialHint: 'API key from your Contrado account — exact field names not public, confirm with the vendor', fields: [{ label: 'API Key', secret: true }], connectsVia: 'store-pull-woo', note: 'No connection test: no public API documentation was found, so any endpoint would be a guess. Confirm the field with Contrado before relying on it.' },
