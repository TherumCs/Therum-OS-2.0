...
const gateway = {
    ...
    const created = await pp<{ id: string; status: string; links?: { rel: string; href: string }[] }>(
      cred, '/v2/checkout/orders',
      {
        ...
        body: {
          ...
          // backfills the order.
          //
          // return_url is the whole game for a redirect gateway: approval on
          // PayPal only AUTHORISES — the money moves when the buyer returns to
          // our capture endpoint. This was set ONLY inside the Venmo branch, so
          // a plain PayPal checkout approved and then stranded the buyer on
          // PayPal with the order forever "pending", never captured, never
          // emailed. Setting it here fixes every PayPal funding source at once.
          application_context: {
  ...
  parseEvent(verified: Record<string, unknown>): PspWebhookEvent {
    ...
    return {
      ...
      paymentIntentId: paypalOrderId ?? ourOrderId,
      refundId: type.includes('REFUND') ? String(resource.id ?? '') : null,
      payload: { ...(verified as Record<string, unknown>), ...(ourOrderId ? { orderId: ourOrderId } : {}) },
    };
  },
