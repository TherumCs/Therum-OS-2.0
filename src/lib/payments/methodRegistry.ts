...
export const METHODS: PaymentMethod[] = [
  ...
  // ── Bank ── (hidden: ACH needs Financial Connections bank-linking + an async
  // settlement webhook, neither wired yet. `hidden` keeps it out of the strip
  // entirely and — since it's the only method in the group — drops the whole
  // "Bank" tile, because methods() filters groups with no non-hidden methods.)
  { id: 'bank_ach', group: 'bank', label: 'Bank (Plaid)', providers: ['plaid', 'stripe'], needsRedirect: true, hidden: true, sub: 'Pay directly from your bank account. Saves ~2% in card fees.' },
