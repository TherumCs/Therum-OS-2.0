...
// Query-string parameters that carry a secret. WooCommerce's fallback
// transport (see counter/storeCredentials.ts) and the OAuth1 one-legged form
// put credentials in the URL, which Fastify logs verbatim as `req.url` on
// every request — so the partner's secret would sit in the access log at info
// level. Masked in the request serializer below. Names are matched lowercased.
const SECRET_QS = new Set([
  'consumer_secret', 'consumer_key', 'oauth_signature', 'oauth_consumer_key',
  'oauth_token', 'access_token', 'client_secret', 'token', 'signature',
  'api_key', 'apikey', 'password', 'secret',
]);
function maskSecretQuery(url: string): string {
  const q = url.indexOf('?');
  if (q === -1) return url;
  const masked = url
    .slice(q + 1)
    ...
    .map((kv) => {
      ...
      return SECRET_QS.has(decodeURIComponent(name).toLowerCase()) ? `${name}=***` : kv;
    })
    .join('&');
  return `${url.slice(0, q)}?${masked}`;
}
...
export async function buildServer() {
  const app = Fastify({
    ...
    logger: {
      level: env.LOG_LEVEL,
      // Keep the authorization header and any cart token out of logs, and mask
      // credential-bearing query params in the request URL (Woo/OAuth1 query
      // auth) — the log is otherwise a plaintext copy of partner secrets.
      redact: {
        paths: ['req.headers.authorization', 'req.headers.cookie', 'req.headers["x-cart-token"]'],
        censor: '***',
      },
      serializers: {
        req(req) {
  ...
  // Raw binary image bodies — the classic WordPress media upload (POST
  // /wp/v2/media) sends the file as the request BODY with a Content-Disposition
  // filename, not as multipart. Buffer those so the wooCompat media route can
  // read req.body as a Buffer. Scoped to image + octet types, so no other route
  // is affected.
  app.addContentTypeParser(
    ['application/octet-stream', 'image/jpeg', 'image/png', 'image/webp', 'image/gif', 'image/avif'],
    { parseAs: 'buffer' },
...
await app.register(shopifyOAuthRoutes);
  ...
  app.addHook('onRequest', async (req, reply) => {
    ...
    const EXEMPT = ['/tos-admin', '/api', '/auth', '/builder', '/wp-json', '/wc-auth', '/admin', '/uploads', '/favicon', '/feed'];
