...
export const STOCK_STATUSES: StockStatus[] = ['tracked', 'in_stock', 'out_of_stock', 'backorder'];

export function isStockStatus(v: unknown): v is StockStatus {
  return typeof v === 'string' && (STOCK_STATUSES as string[]).includes(v);
}
...
export function isSellable(v: StockLike, quantity = 1): boolean {
  return availableOf(v) >= quantity;
}
