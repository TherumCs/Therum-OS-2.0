...
async function upsertCustomer(email: string, name?: string) {
  // Case-insensitive so an already-lowercased sign-in email still matches a
  // verbatim mixed-case stored row (migrated/admin-created accounts) instead
  // of minting a duplicate account that loses member pricing + order history.
  const existing = await db.customer.findFirst({ where: { email: { equals: email, mode: 'insensitive' } } });
...
export const customerAuth = {
  ...
  async resetPasswordWithCode(input: { email: string; code: string; newPassword: string; ip?: string }) {
    ...
    const row = await db.customerAuthCode.findFirst({
      where: { destination: email, kind: 'email', consumedAt: null, expiresAt: { gt: new Date() } },
    ...
    if (identity) {
      ...
      await db.customerIdentity.create({ data: { customerId: customer.id, kind: 'password', subject: email, secretHash, verifiedAt: new Date() } });
    }
  ...
  async requestCode(input: { destination: string; kind: 'phone' | 'email'; ip?: string }) {
    ...
    if (input.kind === 'email') {
      // Deliver the code. Best-effort, fire-and-forget: a dead mail transport
      // must never block or fail a sign-in. Before this the code was generated
      // and stored but never sent — the route voided it — so email sign-in and
      // the emailed change-email step could never actually complete.
