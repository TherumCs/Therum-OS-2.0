 ...
 * it paid. Each entry also carries the order's customer — the linked account's
 * name/email, or the guest checkout email — so a row can name who paid.
 */
async function ordersByIntent(intents: string[]): Promise<Map<string, { number: string; id: string; customer: string | null }>> {
  ...
  const map = new Map<string, { number: string; id: string; customer: string | null }>();
  for (const r of rows) {
    if (r.txnId && r.order) {
      // A real identity only: the linked customer account's name, then any
      // email on the order (account or guest checkout). Never fabricated —
      // null when the order carries no name or email at all.
      const customer = r.order.customer?.name || r.order.customer?.email || r.order.guestEmail || null;
      map.set(r.txnId, { number: r.order.number, id: r.order.id, customer });
    }
  }
  return map;
}
  ...
  async overview() {
    ...
    try {
      ...
      return {
        ...
        notices,
        // Manage-in-Stripe deep links. the store's Stripe is its own account
