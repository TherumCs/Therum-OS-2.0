...
export const clusterService = {
  ...
  // Full merged variant ROWS for the storefront. Same merge rule as
  // resolveMerged (one entry per colour+size, primary-first, an in-stock source
  // winning the tie), but returns the real ProductVariant rows — images,
  // colorCodes, price, everything the PDP and cards render — plus the cluster's
  // display name and the primary's slug. The storefront feeds these straight
  // into its variant-driven picker, so a clustered set renders as ONE product
  // page and ONE grid tile. Returns null for an unknown or dissolved (<2) group.
  async mergedForStorefront(groupId: string) {
    const g = await db.clusterGroup.findUnique({
      where: { id: groupId },
      include: { members: { include: { product: { select: { id: true, slug: true, image: true, images: true, createdAt: true, status: true, deletedAt: true, variants: true } } } } },
    });
    if (!g || g.members.length < 2) return null;
    const primaryId = pickPrimary(g.members, g.primaryProductId);
    const primary = g.members.find((m) => m.productId === primaryId)?.product ?? null;
    const byCombo = new Map<string, (typeof g.members)[number]['product']['variants'][number]>();
    for (const m of orderMembers(g.members, primaryId ?? '')) {
      // A trashed or unpublished member drops out rather than 404ing when its
      // colourway is picked, or advertising stock that cannot be bought.
      if (m.product.deletedAt || m.product.status !== 'active') continue;
      for (const v of m.product.variants) {
        // A colourway's photo lives on the MEMBER PRODUCT, not on its size
        // variants — so carry it onto each merged variant. Without this the
        // swatch has no image and the PDP's showVariantShots bails on !v.image,
        // so picking a colour changed nothing on screen.
        const vi = {
          ...v,
          image: v.image ?? m.product.image ?? null,
          images: (Array.isArray(v.images) && v.images.length) ? v.images : (m.product.images ?? []),
        };
        const key = comboKey(v.color, v.size);
        const cur = byCombo.get(key);
        if (!cur) byCombo.set(key, vi);
        else if (availableOf(cur) <= 0 && availableOf(vi) > 0) byCombo.set(key, vi);
      }
    }
    return { name: g.name, primaryProductId: primaryId ?? '', primarySlug: primary?.slug ?? null, variants: [...byCombo.values()] };
  },
