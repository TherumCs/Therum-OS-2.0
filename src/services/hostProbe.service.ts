...
/** Masks a secret to a short fingerprint (first 4 + last 2 + length). Probes
 *  today keep secrets out of the payload by reporting only hostnames (hostOf)
 *  and loopback state, so nothing calls this at capture; it is a helper kept
 *  available for a probe that must surface a value, exercised by the
 *  host-advisor test. (Earlier the doc claimed redaction happened here at
 *  capture — it did not, and that claim is removed.) */
