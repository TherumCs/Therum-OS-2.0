...
async function resolveDumpRunner(): Promise<DumpRunner | null> {
  try {
    ...
    // Same generous buffer as the docker path below. The default is 1MB, which
    // the database outgrew — every nightly scheduled backup then died with
    // ERR_CHILD_PROCESS_STDIO_MAXBUFFER and the store ran with no backups.
    return { label: 'host pg_dump', run: (args) => execFileAsync('pg_dump', args, { maxBuffer: 512 * 1024 * 1024 }) };
  } catch {
...
export const backupService = {
  async runNow(): Promise<BackupFile> {
    ...
    // Retention: keep only the newest N local backups. Without this, every run
    // re-bundles all of uploads/ into a fresh zip and NONE are ever deleted, so
    // backups/ grows until the disk fills and backups silently stop. Stamp-based
    // filenames sort lexicographically = chronologically. Best-effort: never let
    // cleanup fail a backup that already succeeded. (Runs before S3 so it still
    // happens when the upload throws.)
    try {
      const KEEP = 10;
      const zips = (await readdir(BACKUP_DIR)).filter((n) => n.endsWith('.zip') && n !== filename).sort();
      for (const old of zips.slice(0, Math.max(0, zips.length - (KEEP - 1)))) {
