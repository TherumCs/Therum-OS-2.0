...
// Every test is capped: a provider whose host is down or a wrong URL a shopper
// pasted must FAIL the test after a bounded wait, never leave the spinner
// turning forever. A timeout surfaces as "timed out", not a generic error, so
// "it just hangs" becomes a readable result.
const TEST_TIMEOUT_MS = 12_000;
function timeoutSignal(): AbortSignal | undefined {
  return AbortSignal.timeout ? AbortSignal.timeout(TEST_TIMEOUT_MS) : undefined;
}
function reachError(e: unknown): TestResult {
  const msg = e instanceof Error ? e.message : 'Network error';
  const timedOut = e instanceof Error && (e.name === 'TimeoutError' || e.name === 'AbortError');
  return { ok: false, detail: timedOut ? `No response after ${TEST_TIMEOUT_MS / 1000}s — check the store URL is reachable.` : msg };
}
...
async function post(url: string, headers: Record<string, string>, body: string | null = null): Promise<TestResult> {
  try {
    const res = await fetch(url, { method: 'POST', headers, signal: timeoutSignal(), ...(body === null ? {} : { body }) });
    return { ok: res.ok, detail: res.ok ? `${res.status} OK` : `${res.status} ${res.statusText}` };
  } catch (e) {
    return reachError(e);
  }
...
const wooStyle: Tester = async (c) => {
  // store-pull-woo partners all carry the SAME credential shape: website|key|
  // secret. The pair is authenticated against the WooCommerce REST API at the
  // website in the credential — every store-pull partner answers /wp-json/wc/v3
  // with a consumer key/secret.
  //
  // This was hardcoded to api.printful.com/stores and ignored the website
  // entirely, so testing ANY non-Printful partner (Tapstitch, PODpartner,
  // Gooten, SPOD, Merchize, JetPrint, PODpluser) sent the wrong key to
  // Printful and 401'd — a working connection reported as an error.
  ...
  if (!/^https?:\/\//i.test(base)) {
    return { ok: false, detail: 'Expected the store Website (https://…) as the first value.' };
  }
  return get(`${base}/wp-json/wc/v3/products?per_page=1`, { Authorization: basicAuthHeader(key, secret) });
};
