...
import { shippingRateService, type ShippingRate } from '../counter/shippingRates.js';
...
interface CartState {
  ...
  /** First-touch traffic source (utm_source / referrer host), stamped once on
   *  the first add-to-cart and carried onto the order for attribution. */
  source?: string | null;
  createdAt: string;
}
...
export interface CartTotals {
  ...
  // `pct` is carried so checkout can hand the ORDER the exact same figure the
  // cart computed (margin-floor clamped) instead of the order recomputing it.
  discount: { amount: number; label: string; pct: number } | null;
  ...
  total: number;
  /**
   * Every rate this destination can pick from, cheapest first — the picker's
   * source of truth. Populated only once shipAddress is known; [] before that.
   * It is the SAME list computeTotals charges from, so the price the shopper
   * selects and the price the summary shows can never disagree.
   */
  shippingOptions: ShippingRate[];
  /** The id within shippingOptions that `shipping` was charged from. */
  shippingMethodId: string | null;
}
...
async function computeTotals(state: CartState): Promise<CartTotals> {
  ...
  const variants = variantIds.length
    ? await db.productVariant.findMany({
        ...
        include: { product: { select: { name: true, slug: true, image: true, status: true, fulfillmentProvider: true, meta: true } } },
      })
    : [];
  ...
  // Same guard order.create enforces — see src/counter/marginFloor.ts. Sharing
  // the function is what keeps the figure the cart shows and the figure the
  // order charges identical.
  const maxDiscount = maxDiscountForMargin(
    lines.map((l) => ({ lineTotal: l.lineTotal, quantity: l.quantity, cost: byId.get(l.variantId)?.cost ?? null })),
    subtotal,
    commerce.minMarginPct,
  );
  ...
  const ctx = contextFor(
    lines.map((l, i) => {
      ...
      return {
        ...
        // Fed to shippingRateService.rates(): fulfilment provider drives the
        // single-vendor rule, the variant's source id keys a provider quote,
        // pickup eligibility gates local pickup.
        vendor: v.product.fulfillmentProvider ?? null,
        pickupEligible: pmeta?.pickupEligible === true,
        sourceVariantId: v.sourceId ?? null,
      };
    }),
    storeCurrency,
  );
  ...
  let shippingOptions: ShippingRate[] = [];
  let shippingMethodId: string | null = null;
  if (state.shipAddress) {
    shippingOptions = await shippingRateService.rates({
      lines: ctx.lines,
      subtotal,
      currency: storeCurrency,
      address: state.shipAddress,
    });
    const chosen = shippingOptions.find((r) => r.id === state.shippingMethodId) ?? shippingOptions[0];
    if (chosen) {
      ...
      ctx.tax =
        typeof chosen.taxAmount === 'number'
          ? chosen.taxAmount
          : await shippingRateService.tax(Math.max(0, subtotal - (discount?.amount ?? 0) - (coupon?.amount ?? 0)));
    }
  }
...
export const cartService = {
  ...
  async setShipping(token: string, shipAddress: ShipAddressInput, methodId?: string) {
    ...
    const valid = probe.shippingOptions.find((r) => r.id === methodId);
    state.shippingMethodId = valid ? valid.id : (probe.shippingMethodId ?? null);
    await save(state);
  ...
  async checkout(token: string, email?: string, customerId?: string, shipAddress?: ShipAddressInput) {
    ...
    // Reconcile against what actually priced: a variant hard-deleted after it
    // was added is hidden from totals.lines but still sits in state.items, and
    // passing it to order.create hard-fails with "Unknown variant" — a silent
    // uncheckoutable dead end. Prune vanished lines from the cart, fail cleanly
    // if none survive, and build the order from the live priced lines below.
    const liveIds = new Set(totals.lines.map((l) => l.variantId));
    if (state.items.some((i) => !liveIds.has(i.variantId))) {
      state.items = state.items.filter((i) => liveIds.has(i.variantId));
      await save(state);
    }
    if (totals.lines.length === 0) throw new ValidationError('The items in your cart are no longer available.', 'cart');
    ...
    const baseOrder = await orderService.create({
      ...
      // Hand the order the member discount the cart already resolved (margin-floor
      // clamped, per-product opt-outs applied) so it charges exactly what was
      // displayed. `null` = no member discount (guest, or a fully opted-out
      // basket) — still authoritative, so create() never recomputes an
      // unclamped figure of its own.
      discountOverride: totals.discount
        ? { amount: totals.discount.amount, pct: totals.discount.pct, label: totals.discount.label }
        : null,
      ...(state.source ? { source: state.source } : {}),
      items: totals.lines.map((l) => ({ variantId: l.variantId, quantity: l.quantity })),
    });
    ...
    if (totals.coupon && state.couponCode) {
      ...
      if (q) {
        // Apply the margin-floor-clamped amount the cart DISPLAYED (totals.coupon),
    ...
    // The cart is NOT cleared here. /cart/checkout only CREATES the (pending)
    // order — it takes no money. Clearing now meant a declined or abandoned
    // payment left the shopper with an empty cart and a retry that 404'd
    // ("Cart not found"), an unrecoverable dead end on every rail. The cart is
    // cleared instead at the confirmed-payment edge (orderService.markPaid),
    // and a retry safely reloads this same cart → the same pending order via
    // the `cart_${state.id}` idempotency key (no double reservation).
