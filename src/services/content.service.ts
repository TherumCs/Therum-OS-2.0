...
export const contentService = {
  ...
  async renderBySlug(slug: string, origin = '', urlOverride?: string) {
    const item = await this.getBySlug(slug);
    // The homepage is served at the root, so its canonical/og:url must be the
    // origin itself — not origin + '/' + slug, which pointed Meta and search
    // engines at a stale homepage slug, a URL the homepage does not live at.
    const url = urlOverride ?? `${origin}/${item.slug}`;
