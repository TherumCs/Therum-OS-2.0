...
export const AppearanceInput = z.object({
  ...
  sidebarHighlight: z.enum(['band', 'indicator', 'underline']).optional(),
...
export const AdminDockInput = z.object({
  ...
  // Visual skin of the admin lens: compact floating bar (default), a larger
  // floating bar, or a macOS-style icon dock.
  style: z.enum(['bar', 'large', 'dock']).optional(),
});
...
export const ShippingMethodInput = z.object({
  ...
  /** Price per zone id, minor units. 0 = free. A zone missing from the map is
   *  not offered for this method. Legacy flat `amount` is still accepted and
   *  folded into prices['rest'] by the service getter. */
  prices: z.record(z.string(), z.number().int().min(0)).default({}),
  amount: z.number().int().min(0).optional(),
  /** Free when the cart subtotal reaches this (minor units); null = never. */
  freeOver: z.number().int().min(0).nullable().default(null),
  enabled: z.boolean().default(true),
});

/** A named group of US states, priced together. Exactly one zone is the
 *  `isRest` catch-all so a destination never falls through to no rate. */
...
/** Local pickup: a per-product eligibility flag (on the product) plus this one
 *  global address + fee. `slots` optionally splits pickup into multiple named
 *  time/place options that render as separate shipping methods at checkout. */
export const PickupSlotInput = z.object({
  id: z.string().min(1).max(40),
...
export const PaymentsSettingsInput = z.object({
  ...
  /**
   * WHICH account cards / wallets / Stripe-BNPL settle to.
   *
   * 'stripe' = the standalone Stripe account the storefront charged directly
   * (pk_live_51EpG0CG…). 'woopay' = route those charges through the headless
   * WooPayments engine so the money lands in the WooPayments merchant account
   * (the configured merchant account — where the payout bank + instant deposits live).
   * PayPal/Venmo are NEVER affected; they keep their own rail. Default 'stripe'
   * so this is a pure, reversible toggle — flip to 'woopay' to go live, flip
   * back for instant rollback with zero code change.
   */
  cardProvider: z.enum(['stripe', 'woopay']).optional(),
});
// ─── Stealth ───────────────────────────────────────────────────────────────
