...
// Canonical apparel size order for chips, pickers and filters — smallest to
// largest. Sizes come off variants in vendor push order (Tapstitch, Printful,
// …), which is arbitrary, and a plain .sort() puts L before M before S. Every
// list of sizes shown to a shopper runs through sortSizes() first.
const SIZE_SCALE = ['xxxs', 'xxs', 'xs', 's', 'm', 'l', 'xl', 'xxl', 'xxxl', 'xxxxl', 'xxxxxl'];

/** A sortable rank for one size token, smallest first. Handles S/M/L/XL, the
 *  nXL/nXS forms (2XL → XXL), waist/shoe numbers, hat ranges (S/M, L/XL) by
 *  their first token, and One Size. Unknown labels sort after known alpha
 *  sizes but before numeric ones, staying stable by name. */
export function sizeRank(raw: string): number {
  const t = raw.trim().toLowerCase();
  if (/^(one[\s-]?size|os|osfa|o\/s|free[\s-]?size)$/.test(t)) return 900;
  const first = t.split(/[\/\-–]/)[0]!.trim();          // "S/M" → "s", "L-XL" → "l"
  const mult = first.match(/^(\d+)\s*x(l|s)$/);         // "2xl" → xxl, "3xs" → xxxs
  const key = mult ? 'x'.repeat(Number(mult[1])) + mult[2] : first;
  const idx = SIZE_SCALE.indexOf(key);
  if (idx >= 0) return idx;
  const num = Number(first);
  if (!Number.isNaN(num)) return 1000 + num;            // waist/shoe numbers, in value order
  return 800;                                            // unknown label
}

/** Sizes smallest-to-largest, stable by name within an equal rank. */
...
//
// Read off the live reference source rather than invented: the theme styles
...
// A POD vendor (Tapstitch) sends descriptive colour NAMES with no hex — "Oat
// Gray", "Milk Tea", "Sand Color" — which are not valid CSS colours, so the
// swatch used to fall back to a 3-letter text stub ("Oat", "Mil", "Sa").
// Resolve the name to an approximate hex: exact phrase first, then the last
// recognised colour word, so a swatch is always a coloured dot.
const COLOUR_NAMES: Record<string, string> = {
  'oat gray': '#d9cfbc', 'oat grey': '#d9cfbc', oatmeal: '#e0d8c4', 'milk tea': '#c9a985',
  'sand color': '#d8c6a3', 'sand colour': '#d8c6a3', 'blackish green': '#1e2a24',
  'off white': '#f3f0e9', 'gray apricot': '#c8b6a6', 'gray green': '#8b968a', 'gray camel': '#b39b7d',
  'carbon gray': '#3f4247', 'watermalon red': '#c0384b', 'watermelon red': '#c0384b',
  'kelly green': '#3a9d5a', 'midnight green': '#12463f', 'army green': '#4b5320', 'military green': '#4b5320',
};
const COLOUR_WORDS: Record<string, string> = {
  black: '#1a1a1a', white: '#ffffff', gray: '#8a8a8a', grey: '#8a8a8a', green: '#3f7a4d', red: '#b83a3a',
  blue: '#3a5bb8', navy: '#1c2541', brown: '#6b4a34', tan: '#d2b48c', sand: '#d8c6a3', oat: '#d9cfbc',
  camel: '#b39b7d', apricot: '#e6b98f', pink: '#e39ab0', purple: '#7a4ea3', yellow: '#e3c34a',
  orange: '#e08a3a', beige: '#e8dcc0', khaki: '#c3b091', cream: '#f5efe0', olive: '#808000',
  teal: '#2c7a7b', tea: '#c9a985', milk: '#e8ddca', charcoal: '#36454f', maroon: '#800000', burgundy: '#800020',
};
export function colourToHex(name: string): string | null {
  const n = name.toLowerCase().trim();
  if (COLOUR_NAMES[n]) return COLOUR_NAMES[n]!;
  for (const w of n.split(/[^a-z]+/).filter(Boolean).reverse()) if (COLOUR_WORDS[w]) return COLOUR_WORDS[w]!;
  return null;
}
...
function swatch(color: string, codes: string[] = []): string {
  ...
  const hex = colourToHex(color);
  if (hex) return `<span class="card-swatch" style="background:${hex}" title="${esc(color)}" aria-label="${esc(color)}"></span>`;
...
export function productCard(p: GridProduct, cfg: CardConfig = CARD_DEFAULTS, perRow = 4): string {
          ...
          <div class="card-pay__receipt" data-pay-receipt hidden>
            ...
            <section class="card-pay__sec" data-pay-sec="who">
              ...
              <div class="card-pay__sec-body" data-pay-sec-body="who">
                ...
                <input class="card-pay__in" data-pay-phone type="tel" inputmode="tel" placeholder="Phone (optional)" autocomplete="tel">
                ...
                <div class="card-pay__row">
                  ...
                  <!-- A <select>, never a maxlength=2 text input: autofill drops
                       the full country name into a coded field and dead-ends Pay.
                       Shared list so it can't drift from the checkout surface. -->
                  ${countrySelect('class="card-pay__in" data-pay-country autocomplete="shipping country" aria-label="Country"')}
                </div>
  ...
  const swatches = rows.swatches && p.colors?.length
    ...
    : '';
  // The BUY-FLOW picker shows EVERY colour, scrollable — no 6-cap, no "+N"
  // badge — so a shopper can reach colour 7+ exactly the way the size row lets
  // them reach every size. (The compact card-tile row above keeps its cap.)
  const pickerSwatches = p.colors?.length
    ? `<div class="card-swatches" data-swatch>${p.colors.slice(0, 24).map((c) => interactiveSwatch(c, p.colorCodes?.[c] ?? [])).join('')}</div>`
    : '';
  ...
  const cardPicker = (hasColourChoice || hasSizeChoice) && !soldOut
    ? `<div class="card-picker-reveal">`
      ...
      + (hasColourChoice ? `<div class="card-pk-line card-pk-line--color"><span class="card-pk-label">Color</span><div class="card-pk-scroll">${pickerSwatches}</div></div>` : '')
...
(function(){
  ...
  document.querySelectorAll('[data-evolve]').forEach(function(root){
    ...
    function say(msg, bad){
      ...
      // An ERROR nobody sees reads as a dead button — the message line sits at
      // the bottom of the panel, often below the fold on a phone. Instant, not
      // smooth (smooth rides animation frames). Info messages don't yank the
      // page around; only failures do.
      if (m && bad && msg && m.scrollIntoView) { try { m.scrollIntoView({ block: 'center' }); } catch (x) {} }
    }
    ...
    function addr(){
      ...
      var ph = v('[data-pay-phone]'); if (ph) a.phone = ph;
      return a;
    }
    ...
    async function api(path, body){
      var res = await fetch('/api' + path, {
        ...
        // A request that hangs inside the Apple Pay callback leaves the native
        // sheet on "Processing…" until the OS gives up (~30s). Time out first so
        // the error path runs ev.complete('fail') and the sheet says so.
    ...
    async function pay(){
      ...
      try {
        ...
        var token = null;
        // WooPayments confirms the card at SETTLE (from a client_secret); there
        // is no token. Only Stripe/Square tokenize up front, as before.
        if (CARD_RAIL !== 'woopay') try {
        ...
        // Bill the option the button quoted (quotedShipId), so the charge equals
        // the "Pay $X" the shopper agreed to; the server re-verifies it.
        await api('/cart/shipping', { cartToken: cart.token, shipAddress: a, methodId: quotedShipId || undefined });
        ...
        if (CARD_RAIL === 'woopay') {
          // Confirm the mounted card against the WooPayments intent in-page,
          // then settle. Same success face as the token path.
          var wr = await settleWoopay(order.orderNumber, order.accessToken, 'card', { cardEl: sqCard }, a, email);
          if (wr === 'cancelled') { openSec('payment'); say(''); if (go) { go.disabled = false; go.textContent = 'Pay'; } return; }
          fillDone(order.orderNumber, order.accessToken, email);
          say(''); payStep('done');
          if (go) { go.disabled = false; go.textContent = 'Pay'; }
          if (resetTimer) clearTimeout(resetTimer);
          resetTimer = setTimeout(function(){ resetCard(true); say('Order ' + order.orderNumber + ' confirmed — check your email for the details.'); }, 12000);
          return;
        }
        if (token && payReady) {
    ...
    function syncPayBtn(){
      ...
      var why = !chosenVariant ? 'Choose an option first'
        ...
        : '';
      go.disabled = !!why;
      go.textContent = why || ('Pay ' + money(chosenVariant.p * qty + quotedShip));
    }

    // ── EXPRESS SHIPPING QUOTE ──────────────────────────────────────────────
    // The quick-buy shows a running "Pay $X". Once shipping can cost something
    // (a Printful rate, a paid zone), X must include it or the shopper is billed
    // more than the button promised. Quote the destination the moment the
    // address is complete, fold in the cheapest option, and send that same
    // option id at checkout so the charge equals the button. Deduped by the only
    // things that move a rate — variant, quantity, region, postcode, country —
    // so a name keystroke does not hit the network.
    var quotedShip = 0, quotedShipId = null, shipQuoteKey = '';
    async function quoteExpressShip(){
      if (!chosenVariant) return;
      var a = addr();
      if (!a.name || !a.line1 || !a.city || a.country.length !== 2) return;
      var key = chosenVariant.i + '|' + qty + '|' + (a.region || '') + '|' + (a.postalCode || '') + '|' + a.country;
      if (key === shipQuoteKey) return;
      shipQuoteKey = key;
      try {
        var out = await api('/shop/shipping/rates', { items: [{ variantId: chosenVariant.i, quantity: qty }], address: a });
        var pick = (out.rates || [])[0];
        quotedShip = pick ? pick.amount : 0;
        quotedShipId = pick ? pick.id : null;
      } catch (e) { quotedShip = 0; quotedShipId = null; }
      drawLine();
    }
    ...
    var walletPrequote = null;        // shipping option pre-quoted before the sheet opens
    // FLIGHT RECORDER — fire-and-forget stage pings, so a failing sheet on a
    // real phone names its dying step in the server log instead of leaving us
    // to guess from silence. sendBeacon, so a closing sheet can't cancel it.
    function wlog(stage){
      try {
    ...
    async function initStripeWallets(providers, box){
      var sp = (providers || []).filter(function(p){ return p.provider === 'woopay' && p.ready; })[0]
            || (providers || []).filter(function(p){ return p.provider === 'stripe' && p.ready; })[0];
      var key = sp && sp.client && sp.client.publishableKey;
      if (!key || !chosenVariant || !box) return;
      try {
        await loadSdk('https://js.stripe.com/v3/');
        // The PaymentRequest's pm must belong to the account that confirms it —
        // build against the WooPayments connected account when that's the rail.
        var stripe = (sp.provider === 'woopay' && sp.client.stripeAccount)
          ? window.Stripe(key, { stripeAccount: sp.client.stripeAccount })
          : window.Stripe(key);
        ...
        // The sheet's subtotal for this single-product quick buy. Read live so a
        // quantity bump before the sheet opens is reflected.
        function walletSubtotal(){ return Math.max(1, chosenVariant.p * qty); }
        // Real rates for THIS item + the address the sheet reveals, mapped to the
        // shape Stripe's sheet wants. Same engine the card checkout charges from,
        // so the amount the wallet authorises is the amount we bill.
        // The sheet's shipping option is quoted BEFORE the sheet ever opens,
        // and every Apple callback below is answered SYNCHRONOUSLY. The old
        // shape fetched rates inside shippingaddresschange — an async network
        // round-trip in the middle of the native sheet, and the exact step
        // after which a real iPhone's sheet died stuck on "Processing…". The
        // shown price is the country-level cheapest; the server re-quotes the
        // real address at order time and bills its own cheapest, so the figures
        // agree in all but exotic cases. NO network inside the sheet, ever.
        walletPrequote = freeShip[0];
        api('/shop/shipping/rates', {
          items: [{ variantId: chosenVariant.i, quantity: qty }],
          address: { line1: 'x', city: 'x', region: '', postalCode: '', country: 'US' },
        }).then(function(out){
          var r = (out.rates || [])[0];
          if (r) walletPrequote = { id: 'standard', label: r.name, detail: r.detail || '', amount: r.amount };
        }).catch(function(){ /* freeShip stands */ });
        stripePR = stripe.paymentRequest({
          country: 'US', currency: 'usd',
          total: { label: productLabel(), amount: walletSubtotal() },
        ...
        // any money moves. Everything else answers instantly from the prequote.
        stripePR.on('shippingaddresschange', function(ev){
          wlog('card:addresschange');
          var sa = ev.shippingAddress || {};
          if ((sa.country || '').toUpperCase() !== 'US') {
            return ev.updateWith({ status: 'invalid_shipping_address' });
          }
          ev.updateWith({ status: 'success', shippingOptions: [walletPrequote],
            total: { label: productLabel(), amount: walletSubtotal() + (walletPrequote.amount || 0) } });
        });
        stripePR.on('shippingoptionchange', function(ev){ ev.updateWith({ status: 'success' }); });
        stripePR.on('cancel', function(){ wlog('card:cancel'); });
        stripePR.on('paymentmethod', function(ev){ wlog('card:paymentmethod'); onWalletPaymentMethod(ev); });
    ...
    async function onWalletPaymentMethod(ev){
      // ONE completion, and never later than the OS's own patience. The settle
      // path is four sequential calls; each carries its own 15s cap, but the
      // sum could outlive the ~30s the native sheet allows. This watchdog
      // fails the sheet at 25s — the guard makes whichever side loses the race
      // a no-op, so a late success can't double-complete.
      var settled = false;
      function complete(state){ if (settled) return false; settled = true; try { ev.complete(state); } catch (x) {} return true; }
      var watchdog = setTimeout(function(){
        wlog('card:watchdog-fired');
        if (complete('fail')) say('That took too long — check your email for an order confirmation before trying again.', true);
      }, 25000);
      try {
        ...
        if (!a.line1 || !a.city || a.country.length !== 2) {
          ...
          if (complete('invalid_shipping_address')) say('That address was incomplete — try another card or the form.', true);
          return;
        }
        var email = ev.payerEmail || '';
        var cart = await api('/cart/items', { variantId: chosenVariant.i, quantity: qty });
        // No methodId: the sheet showed the pre-quoted cheapest, and the server
        // bills ITS cheapest for the real address — the same figure in all but
        // exotic cases, and never an id from a stale quote.
        ...
        if (CARD_RAIL === 'woopay') {
          ...
          await settleWoopay(order.orderNumber, order.accessToken, 'apple_pay', { pmId: ev.paymentMethod.id }, a, email);
        } else {
          await api('/shop/checkout/pay-token', {
            orderNumber: order.orderNumber, accessToken: order.accessToken,
        ...
        // lingers over the card. If the watchdog already failed the sheet, the
        // charge still landed — paint the confirmation anyway; the order number
        // is the truth the shopper needs.
        complete('success');
        drawLine();
        fillDone(order.orderNumber, order.accessToken, email);
        say('');
        ...
        clearTimeout(watchdog);
        wlog('card:settle-error:' + String(e && e.message || e).slice(0, 60));
        if (complete('fail')) {
          payStep('who');
          say(e.message || String(e), true);
        }
    ...
    function payWallet(){
      ...
      // NO await between the tap and show(): iOS only opens the Apple Pay sheet
      // when it is called synchronously inside the user gesture — even a
      // resolved promise's microtask breaks the chain and the sheet wedges.
      // The wallets init when the card renders, so by tap time they exist; a
      // tap that beats the init just asks for a second tap.
      if (!stripePR || !stripeCan) {
        say('One second — payment is still warming up. Tap again.', true);
        return;
      }
      try {
        var ship = (walletPrequote && walletPrequote.amount) || 0;
        stripePR.update({ total: { label: productLabel(), amount: Math.max(1, chosenVariant.p * qty + ship) },
          shippingOptions: walletPrequote ? [walletPrequote] : undefined });
        wlog('card:show');
        stripePR.show();
      } catch (e) {
        wlog('card:show-error:' + String(e && e.message || e).slice(0, 60));
    ...
    // Which account the card/wallet settles to. 'stripe' = direct (tokenize →
    // pay-token). 'woopay' = confirm in-page against the WooPayments account
    // (no token; a client_secret from the engine). Set from the ready provider.
    ...
    function loadSdk(src){
      return new Promise(function(resolve, reject){
        ...
        // Cap the wait — a CDN that never answers otherwise wedges whichever
        // flow awaited it. A late-landing script still counts: the next call
        // resolves off the already-present check above.
        var t = setTimeout(function(){ reject(new Error('Payment is taking too long to load — try again.')); }, 15000);
        el.src = src; el.async = true;
        el.setAttribute('data-pay-sdk', src);
        el.onload = function(){ clearTimeout(t); resolve(); };
        el.onerror = function(){ clearTimeout(t); reject(new Error('Could not load the payment SDK.')); };
    ...
    async function mountCardField(){
      ...
      try {
        ...
        } else if (payReady.provider === 'woopay' && c.publishableKey && c.stripeAccount) {
          // WooPayments: Stripe.js MUST be built against the connected account
          // or the intent (which belongs to it) cannot be confirmed. The card is
          // confirmed at settle from a client_secret — there is no token here.
          await loadSdk('https://js.stripe.com/v3/');
          var wstripe = window.Stripe(c.publishableKey, { stripeAccount: c.stripeAccount });
          sqCard = wstripe.elements().create('card');
          host.hidden = false;
          sqCard.mount(host);
          sqCard._stripe = wstripe;
          CARD_RAIL = 'woopay';
    ...
    /* Settle a buy-box order through WooPayments: the engine mints an intent,
       the browser confirms it in-page against the connected account (card via
       the mounted field, wallet via the PaymentRequest pm id), then we finalise
       (poll → markPaid) rather than wait on the async engine webhook. Returns
       'ok' | 'cancelled'; throws on decline. source = {cardEl} | {pmId}. */
    async function settleWoopay(orderNumber, accessToken, method, source, ship, email){
      var started = await api('/shop/checkout/redirect-start', {
        orderNumber: orderNumber, accessToken: accessToken, provider: 'woopay', method: method });
      if (!started.clientSecret || !started.publishableKey || !started.stripeAccount) {
        throw new Error('WooPayments could not start this charge. Refresh and try again.');
      }
      await loadSdk('https://js.stripe.com/v3/');
      ...
      var pm = (source && source.pmId) ? source.pmId : { card: source.cardEl, billing_details: billing };
      var res = await stripe.confirmCardPayment(started.clientSecret, { payment_method: pm });
      if (res && res.error) {
        if (/cancel|abort/i.test(res.error.message || '')) return 'cancelled';
        throw new Error(res.error.message);
      }
      try { await api('/shop/checkout/redirect-finish', { orderNumber: orderNumber, accessToken: accessToken, provider: 'woopay' }); } catch (e) { /* webhook backup */ }
      return 'ok';
    }
        ...
        box.addEventListener('click', function(e){
          ...
          if (b && !b.disabled) {
            ...
            if (ex) {
              ...
              // Token wallets (Stripe) AND confirm wallets (WooPayments) both
              // open the browser PaymentRequest sheet — never the card form.
              if (ex.provider === 'stripe' || ex.provider === 'woopay') return payWallet();
    ...
    async function loadWallets(){
      ...
      try {
        ...
        box.hidden = false;
        // One wallet is not a row of discs, it's THE way to pay — .is-solo
        // makes it a labeled full-width button. The class was styled but
        // nothing ever applied it, so a PayPal-only store showed one giant
        // floating disc.
        box.classList.toggle('is-solo', buttons.length === 1);
    ...
    async function mountAddress(){
      ...
      try {
        ...
        // The key rides under client.publishableKey (like every other call
        // site) — reading it off the provider row directly was always
        // undefined, so the address autocomplete silently never mounted.
        var key = sp && sp.client && sp.client.publishableKey;
    ...
    async function loadClientAndMount(provider){
      try {
        ...
        // When the card rail is WooPayments, the stripe-backed card/wallet
        // methods settle through it — prefer the ready woopay entry over the
        // requested (registry) provider id. Falls back to the exact id (and thus
        // the old Stripe/Square behaviour) when no woopay rail is exposed.
        var p = provs.filter(function(x){ return x.provider === 'woopay' && x.ready; })[0]
             || provs.filter(function(x){ return x.provider === provider; })[0];
    ...
    function resetCard(keepMessage){
      ...
      if (!keepMessage) say('');
      quotedShip = 0; quotedShipId = null; shipQuoteKey = '';
      face('rest');
    }
    ...
    if (payEl) {
      ...
      function advance(){
        ...
        // Address is complete — quote its shipping so Payment opens with the
        // real total on the button rather than a price that jumps at checkout.
        quoteExpressShip();
        openSec('payment');
      }
      ...
      payEl.addEventListener('input', function(){ syncPayBtn(); quoteExpressShip(); });
      ...
      payEl.addEventListener('click', function(e){
        ...
        // rather than a dead button. WooPayments (kind 'confirm') rides the same
        // PaymentRequest sheet — its PR is built against the connected account.
        if (payProvider === 'stripe' || payProvider === 'woopay') {
...
/* FIXED-SIZE discs, deliberately not a flex share + aspect-ratio: as flex
   items the row's default stretch alignment gave them a height that ignored
   the ratio on Safari, and the "circles" rendered as ovals on the one surface
   wallets matter most. A hard 64px square with radius 50% is a circle
   everywhere, full stop. */
.card-pay__icons{justify-content:center;align-items:center;gap:12px;padding:2px 4px;flex-wrap:wrap}
.card-pay__icons .card-pay__wallet{color:var(--tx,#111);flex:0 0 auto;
