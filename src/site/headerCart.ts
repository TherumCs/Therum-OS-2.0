...
(function(){
  ...
  window.__thCartClose = function(){ close(); };
  ...
  function open(){
    ...
    if (SIDEBAR && drawer) {
      ...
      // Push needs room to slide the page into. Below 768px the drawer is
      // nearly full-width — nothing to reveal — so push MUST degrade to cover,
      // and the body/html class has to follow the SAME gate as the transform.
      // The old code shifted the page only at >=768 but still tagged the body
      // th-cart-open (push mode) on a phone: no shift happened, so the drawer
      // sat at opacity:0 underneath an unmoved page and the bag "did nothing".
      var pushNow = PUSH && shell && window.innerWidth >= 768;
      if (pushNow) {
      ...
      document.body.classList.add(pushNow ? 'th-cart-open' : 'th-cart-over');
      if (pushNow) document.documentElement.classList.add('th-cart-open');
    } else {
