...
// page: cluster collapse and the price/best-selling sorts are only correct
// over the full result, and pagination then slices it in memory. Far above any
// real catalogue; a hit is logged, never a silent truncation.
const SHOP_MATCH_CAP = 1000;

// Category pills for a card / PDP, per the rule: TOP-LEVEL first, then the leaf
// type. A unisex tee is filed under Men's › T-Shirts AND Women's › T-Shirts, so
// it reads "Men's · Women's · T-Shirts" (parents deduped, one type tag); a
// cushion reads "Accessories · Cushions". Each pill's `slug` is a PATH so the
// href resolves — a leaf shared across parents (t-shirts) would be ambiguous as
// a bare slug, so it links through the first parent it appeared under.
type CatWithParent = { name: string; slug: string; parent: { name: string; slug: string } | null };
function categoryPills(cats: CatWithParent[]): { name: string; slug: string }[] {
  const tops = new Map<string, { name: string; slug: string }>();
  const leaves = new Map<string, { name: string; slug: string }>();
  for (const c of cats) {
    if (c.parent) {
      tops.set(c.parent.name.toLowerCase(), { name: c.parent.name, slug: c.parent.slug });
      if (!leaves.has(c.name.toLowerCase())) leaves.set(c.name.toLowerCase(), { name: c.name, slug: `${c.parent.slug}/${c.slug}` });
    } else {
      // A top-level category the product sits in directly (a line like
      // Essentials, or Accessories itself) is a top-level, not a leaf tag.
      tops.set(c.name.toLowerCase(), { name: c.name, slug: c.slug });
    }
  }
  return [...tops.values(), ...leaves.values()];
}
...
export async function storefrontRoutes(app: FastifyInstance): Promise<void> {
  ...
  const shopPage = async (
    ...
    const query = req.query as { q?: string; category?: string; tag?: string; color?: string; size?: string; sort?: string; brand?: string; stock?: string; price?: string; page?: string };
    ...
    const catalogKey = JSON.stringify({
      ...
      matchCap: SHOP_MATCH_CAP,
      aud: audienceKey,
    });
    // The WHOLE matching set (capped), cached per filter combination. Cluster
    // collapse and the price/best-selling sorts below both need every match,
    // not one page; pagination slices this in memory afterwards.
    const allMatching = await cached('catalog', catalogKey, () => db.product.findMany({
      where: {
        status: 'active',
        // deletedAt:null is NOT optional — trashing a product sets deletedAt but
        // leaves status 'active' (trash is a soft-delete, not an unpublish), so
        // without this a removed product keeps showing on the storefront.
        deletedAt: null,
      ...
      include: { vendor: { select: { name: true } }, variants: true, categories: { select: { name: true, slug: true, parent: { select: { name: true, slug: true } } } }, clusterMembership: { select: { groupId: true } } },
      orderBy,
      take: SHOP_MATCH_CAP,
    }));
    if (allMatching.length >= SHOP_MATCH_CAP) logger.warn({ count: allMatching.length }, 'shop match cap hit — some products not listed; raise SHOP_MATCH_CAP or add cursor paging');

    // Cluster collapse: a clustered set shows ONCE, as its primary, carrying the
    // MERGED colourways so the card shows every swatch. Non-primary members drop
    // out of the grid — their colourway lives on the primary's product page.
    const clusterGroupIds = [...new Set(allMatching.map((p) => p.clusterMembership?.groupId).filter((g): g is string => !!g))];
    const merges = new Map<string, NonNullable<Awaited<ReturnType<typeof clusterService.mergedForStorefront>>>>();
    for (const gid of clusterGroupIds) {
      const m = await clusterService.mergedForStorefront(gid);
      if (m) merges.set(gid, m);
    }
    const collapsed = allMatching.flatMap((p) => {
      const gid = p.clusterMembership?.groupId;
      if (!gid) return [p];
      const m = merges.get(gid);
      if (!m) return [p]; // group dissolved out from under the cache — show as-is
      if (m.primaryProductId !== p.id) return []; // non-primary member drops
      // Shallow copy — the cached objects must never be mutated. The primary
      // carries the cluster's clean name and every member's variants.
      return [{ ...p, name: m.name, variants: m.variants }];
    });

    // Price and best-selling are resolved over the FULL collapsed set, so a
    // page is a real slice of a correct order (not a re-sorted single page).
    if (IN_MEMORY_SORTS.has(sort)) {
      const minPrice = (p: (typeof collapsed)[number]): number => {
        const prices = p.variants.map((v) => v.price);
        return prices.length ? Math.min(...prices) : Number.MAX_SAFE_INTEGER;
      };
      if (sort === 'price-asc') collapsed.sort((a, b) => minPrice(a) - minPrice(b));
      else if (sort === 'price-desc') collapsed.sort((a, b) => minPrice(b) - minPrice(a));
      else {
        const sold = await db.orderItem.groupBy({
          by: ['variantId'],
          where: { variantId: { in: collapsed.flatMap((p) => p.variants.map((v) => v.id)) } },
          _sum: { quantity: true },
        });
        const byVariant = new Map(sold.map((r) => [r.variantId, r._sum.quantity ?? 0]));
        const units = (p: (typeof collapsed)[number]): number =>
          p.variants.reduce((n, v) => n + (byVariant.get(v.id) ?? 0), 0);
        collapsed.sort((a, b) => units(b) - units(a));
      }
    }

    // Numbered pages, `pageSize` to a page. The page is a slice of the
    // collapsed, sorted set; the clamp means ?page=999 lands on the last page
    // rather than an empty one.
    const pageSize = Math.max(1, counter.toolbarPageSize);
    const totalCount = collapsed.length;
    const pageCount = Math.max(1, Math.ceil(totalCount / pageSize));
    const pageNum = Math.min(Math.max(1, Number.parseInt((query.page ?? '1'), 10) || 1), pageCount);
    const products = collapsed.slice((pageNum - 1) * pageSize, pageNum * pageSize);
    ...
    const scope = { status: 'active' as const, deletedAt: null, AND: [visibleWhere(gateViewer)], ...(categoryIds ? { categories: { some: { id: { in: categoryIds } } } } : {}), ...(tag ? { tags: { some: { slug: tag } } } : {}) };
    ...
    const sizes = sortSizes([...new Set(variantAttrs.map((v) => v.size).filter((s): s is string => !!s))]);
    ...
    const qs = (over: Record<string, string>): string => {
      ...
      // page defaults empty so every FILTER link resets to page 1 (a new filter
      // makes the old page number meaningless); only the pager passes it in.
      const next = { q, category, tag, color, size, sort, brand, price: priceBand, stock: inStock ? '1' : '', page: '', ...over };
      for (const [k, v] of Object.entries(next)) {
        // Page 1 is the bare URL — never ?page=1.
        if (k === 'page' && (v === '' || v === '1')) continue;
    ...
    const gridProducts = products.map((p) => {
        ...
        return {
          ...
          // Top-level(s) first, then the type — see categoryPills. "Men's ·
          // Women's · T-Shirts" for a unisex tee, "Accessories · Cushions" for a
          // cushion; never "T-Shirts" twice.
          categories: categoryPills(p.categories),
          ...
          sizes: sortSizes([...new Set(p.variants.map((v) => v.size).filter((z): z is string => !!z))]),
          ...
          // Per-product opt-out (meta.noMemberDiscount): the item shows and
          // charges its flat price, never the member markdown.
          memberPct: (p.meta as Record<string, unknown> | null)?.noMemberDiscount === true ? 0 : memberPct,
    ...
    const cards = productGrid(gridProducts, counter.toolbarColumns, cardCfg);
    ...
    // Label is left RAW — the toolbar escapes it once at render. href is
    // escaped here because the toolbar prints it into the attribute unescaped.
    // (Escaping the label here too double-escaped it: Men's -> Men&#39;s -> the
    // literal text "Men&#39;s" once a category name finally had an apostrophe.)
    const opt = (label: string, href: string, active: boolean): { label: string; href: string; active: boolean } =>
      ({ label, href: esc(href), active });
    ...
    // Numbered pager — only when there's more than one page. A windowed set of
    // numbers (first, current±1, last, with … gaps) so a large catalogue never
    // prints forty links. Filter links reset to page 1 via qs()'s empty default.
    const pageHref = (n: number): string => qs({ page: String(n) });
    const pager = pageCount > 1 ? (() => {
      const items: string[] = [];
      const link = (label: string, n: number, o: { active?: boolean; disabled?: boolean } = {}): void => {
        items.push(o.disabled
          ? `<span class="pager-link is-disabled" aria-hidden="true">${label}</span>`
          : `<a class="pager-link${o.active ? ' is-active' : ''}"${o.active ? ' aria-current="page"' : ''} href="${esc(pageHref(n))}">${label}</a>`);
      };
      link('‹', pageNum - 1, { disabled: pageNum <= 1 });
      const nums = [...new Set([1, pageNum - 1, pageNum, pageNum + 1, pageCount])].filter((n) => n >= 1 && n <= pageCount).sort((a, b) => a - b);
      let prev = 0;
      for (const n of nums) {
        if (n - prev > 1) items.push('<span class="pager-gap" aria-hidden="true">…</span>');
        link(String(n), n, { active: n === pageNum });
        prev = n;
      }
      link('›', pageNum + 1, { disabled: pageNum >= pageCount });
      return '<nav class="pager" aria-label="Pagination"><style>'
        + '.pager{display:flex;gap:6px;justify-content:center;align-items:center;flex-wrap:wrap;margin:30px 0 6px}'
        + '.pager-link{display:inline-flex;min-width:40px;height:40px;padding:0 12px;align-items:center;justify-content:center;'
        + 'border:1px solid var(--bd2,rgba(0,0,0,.16));border-radius:11px;text-decoration:none;color:var(--tx,#111);'
        + 'font-size:14px;font-weight:600;font-variant-numeric:tabular-nums;transition:border-color .15s ease,background .15s ease}'
        + '.pager-link:hover{border-color:var(--tx,#111);background:var(--sf2,#f5f5f5)}'
        + '.pager-link.is-active{background:var(--tx,#111);color:var(--sf,#fff);border-color:var(--tx,#111)}'
        + '.pager-link.is-disabled{opacity:.35;pointer-events:none}'
        + '.pager-gap{padding:0 2px;opacity:.5}'
        + '</style>' + items.join('') + '</nav>';
    })() : '';

    const pageTitle = preset.title ?? 'Shop';
    // Category-page dressing (Template 1): a hero above the grid and a blurb
    // below it, from the per-category config. Only a TOP-LEVEL category page
    // (single-segment path) gets it — subcategory pages stay plain for now.
    const catSlug = preset.category && !preset.category.includes('/') ? preset.category : undefined;
    const catPage = categoryPageFor(catSlug);
    // Full-bleed hero that OPENS at full screen (whole image, title fades up)
    // then SHRINKS to a ~54vh header on first scroll — revealing the search bar
    // and the first product row. margin-top:-40px cancels main{padding-top:40px}
    // so the image butts the nav. --navh (the live header height) is set by the
    // runtime so the open state is exactly one viewport tall, header included.
    const CAT_CSS = '<style>'
      + 'html{overflow-x:clip}'
      + '.cat-hero{position:relative;width:100vw;margin:-40px 0 26px calc(50% - 50vw);min-height:54vh;display:flex;align-items:flex-end;background:var(--tx,#111);background-size:cover;background-position:center;overflow:hidden;transition:min-height .5s cubic-bezier(.4,0,.2,1)}'
      + '.cat-hero--open{min-height:calc(100svh - var(--navh,140px))}'
      + '.cat-hero--open.is-collapsed{min-height:54vh}'
      + '.cat-hero__in{width:100%;max-width:var(--th-site-max,1080px);margin:0 auto;padding:0 24px 40px;box-sizing:border-box}'
      + '.cat-hero__t{font-size:clamp(34px,7vw,64px);font-weight:800;letter-spacing:-.02em;margin:0;line-height:.98;color:#fff;animation:catUp .7s cubic-bezier(.2,.6,.2,1) both}'
      // A logo wordmark CENTERED over the hero image (not replacing the title —
      // the text title still sits in the content row below).
      + '.cat-hero__logo{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:auto;height:auto;max-width:min(420px,55vw);max-height:36vh;z-index:1;pointer-events:none;animation:catLogoUp .7s cubic-bezier(.2,.6,.2,1) both}'
      + '.cat-hero__s{margin:10px 0 0;font-size:16px;color:rgba(255,255,255,.92);animation:catUp .7s .09s cubic-bezier(.2,.6,.2,1) both}'
      // Line two — the longer sentence, sat under the short line like Kith\'s
      // department heroes (title / short line / paragraph). Narrower measure so a
      // long line stays readable over the image.
      + '.cat-hero__s2{max-width:600px;margin:9px 0 0;font-size:14px;line-height:1.55;color:rgba(255,255,255,.82);animation:catUp .7s .16s cubic-bezier(.2,.6,.2,1) both}'
      // Light-background hero (studio/cream shots): dark title, no dark scrim —
      // white-on-cream is unreadable, and the scrim only exists to seat white
      // text over a dark photo.
      + '.cat-hero--light .cat-hero__t{color:#0e0e0e}'
      + '.cat-hero--light .cat-hero__s{color:rgba(0,0,0,.72)}'
      + '.cat-hero--light .cat-hero__s2{color:rgba(0,0,0,.6)}'
      + '@keyframes catUp{from{opacity:0;transform:translateY(26px)}to{opacity:1;transform:none}}'
      // The logo is centered with translate(-50%,-50%); its fade-in must KEEP that
      // translate at both ends, or fill-mode:both leaves it at transform:none and
      // the logo jumps to top-left-anchored (spills down-right). This was the
      // "logo not centered" bug — the shared catUp reset the transform.
      + '@keyframes catLogoUp{from{opacity:0;transform:translate(-50%,calc(-50% + 26px))}to{opacity:1;transform:translate(-50%,-50%)}}'
    ...
    // the very top. Injected inline via layout()'s extraScript (CSP allows it).
    const CAT_HERO_RUNTIME = catPage
      ? '(function(){var h=document.querySelector(".cat-hero--open");if(!h)return;var hd=document.querySelector("#brx-header,header");function nav(){if(hd)document.documentElement.style.setProperty("--navh",Math.round(hd.getBoundingClientRect().height)+"px");}nav();var c=false,t=false;function u(){t=false;var n=(window.scrollY||window.pageYOffset||0)>16;if(n!==c){c=n;h.classList.toggle("is-collapsed",n);}}window.addEventListener("scroll",function(){if(!t){t=true;requestAnimationFrame(u);}},{passive:true});window.addEventListener("resize",function(){if(!t){t=true;requestAnimationFrame(function(){nav();u();});}},{passive:true});u();})();'
      : '';
    // Template selection (from the approved wireframe): a 'collection' category
    // renders the editorial-first layout (no filter bar / pager); a 'grid'
    // category with editorial shots weaves them between product rows; anything
    // else is the plain grid. See src/site/categoryTemplates.ts.
    const isCollection = catPage?.template === 'collection';
    const perRow = catPage?.columns ?? counter.toolbarColumns;
    const emptyState = `<div class="empty-state"><div class="big">🛍️</div><p>No products${q || category || tag || color || size ? ' match those filters' : ' here yet'}.</p>${q || category || tag || color || size ? '<p style="margin-top:12px"><a class="btn ghost sm" href="/shop">Clear filters</a></p>' : ''}</div>`;
    const useTpl = isCollection || (catPage?.template === 'grid' && !!catPage.editorial?.length);
    const catBody = isCollection
      ? renderCollection({ page: catPage!, title: pageTitle, products: gridProducts, perRow, cfg: cardCfg, shopAllHref: '/shop' })
      : catPage?.template === 'grid' && catPage.editorial?.length
        ? weaveDepartment(gridProducts, perRow, cardCfg, catPage.editorial)
        : cards;
  ...
  // Meta Commerce / Google Merchant product feed (RSS 2.0 + g: namespace).
  // Point Meta Commerce Manager / Google Merchant Center at this URL and it
  // pulls the catalog on a schedule (daily). One <item> per sellable variant,
  // grouped by product via g:item_group_id so colours/sizes read as options
  ...
  const feedHandler = async (req: FastifyRequest, reply: FastifyReply): Promise<void> => {
    ...
    for (const p of products) {
      ...
      for (const v of p.variants) {
        ...
        const mainImg = v.image || primary; if (!mainImg) continue;
        // Each variant's OWN extra photos (Meta shows the picked colour's shots),
        // deduped against the main image, capped at 10 per Meta's limit.
        const extraImgs = (Array.isArray(v.images) ? v.images : [])
          .map((x) => (x as { url?: string })?.url)
          .filter((u): u is string => typeof u === 'string' && u.length > 0 && u !== mainImg)
          .slice(0, 10);
        // REAL availability from stock, not a hardcoded "in stock" — a sold-out
        // variant must read as out of stock so Instagram doesn't sell air.
        const inStock = availableOf({ inventory: v.inventory, reserved: v.reserved, stockStatus: v.stockStatus }) > 0;
        const parts: string[] = [
          `<g:id>${esc(v.id)}</g:id>`,
          `<g:item_group_id>${esc(p.id)}</g:item_group_id>`,
          `<g:title>${esc(p.name + ([v.color, v.size].filter(Boolean).length ? ' — ' + [v.color, v.size].filter(Boolean).join(', ') : ''))}</g:title>`,
          `<g:description>${esc(desc)}</g:description>`,
          `<g:link>${esc(pdpLink)}</g:link>`,
          `<g:image_link>${esc(toAbs(mainImg))}</g:image_link>`,
          ...extraImgs.map((u) => `<g:additional_image_link>${esc(toAbs(u))}</g:additional_image_link>`),
          `<g:availability>${inStock ? 'in stock' : 'out of stock'}</g:availability>`,
          `<g:price>${dollars(v.price)}</g:price>`,
          `<g:condition>new</g:condition>`,
          `<g:brand>${esc(brand)}</g:brand>`,
        ];
        // brand + mpn (SKU) is the identifier pair; a SKU-less variant has no
        // unique identifier, so tell the channel that rather than let it flag
        // the item as missing a GTIN.
        if (v.sku) parts.push(`<g:mpn>${esc(v.sku)}</g:mpn>`);
...
<channel>
<title>${esc(process.env.SITE_NAME || '')}</title>
<link>${esc(origin)}</link>
<description>Live product catalogue</description>
${items.join('\n')}
</channel>
</rss>`;
  ...
  app.get('/shop/qr', async (req, reply) => {
    ...
    const origin = originOf(req) || process.env.PUBLIC_ORIGIN || '';
    reply
  ...
  app.get('/product/:slug', async (req, reply) => {
    ...
    const p = await db.product.findUnique({
      ...
      include: {
        ...
        categories: { select: { name: true, slug: true, parent: { select: { name: true, slug: true } } } },
    ...
    // entirely variant-driven (see VARIANTS in the runtime), so feeding it every
    // member's variants — deduped by colour+size, primary-first — puts all
    // colourways on one page. The cluster's name replaces the member's (this `p`
    // is a direct fetch, not a shared cache object, so mutating it is safe), and
    // the canonical points at the primary so the member URLs don't compete.
    const membership = await db.clusterMembership.findUnique({ where: { productId: p.id }, select: { groupId: true } });
    const merged = membership ? await clusterService.mergedForStorefront(membership.groupId) : null;
    if (merged) p.name = merged.name;
    const canonicalSlug = merged?.primarySlug ?? p.slug;
    const variantRows = merged?.variants ?? p.variants;
    ...
    const sizes = sortSizes(uniq(variants.map((v) => v.size).filter((z): z is string => !!z)));
     ...
     */
    const fill = (codes: string[], name?: string): string | null => {
      const safe = codes.filter((c) => /^#[0-9a-f]{3,8}$/i.test(c));
      if (!safe.length) {
        // No vendor hex — Tapstitch sends descriptive colour NAMES only, so a
        // dropdown of them was a row of identical grey dots. Resolve the name to
        // an approximate hex so each dot reads as its actual colour.
        const hex = name ? colourToHex(name) : null;
      ...
      ? `<div class="pdp-box" data-box="color" data-box-toggle="color" role="button" tabindex="0" aria-expanded="false">
           ...
           <div class="pdp-box__pop" data-box-panel="color" hidden>
             <div class="swatches" id="swatches">
               ...
               ${colors.map((c) => {
                const v = firstOf(c)!;
                const paint = fill(v.colorCodes, c);
                const inner = paint
    ...
    // Opt-in per product (meta.quickCheckout): render Add-to-cart AND a Checkout
    // button side by side, so a shopper can go straight to /checkout in one tap.
    // Default (unset) keeps the single Add-to-cart button unchanged.
    const quickCheckout = !!(p.meta && typeof p.meta === 'object' && !Array.isArray(p.meta) && (p.meta as Record<string, unknown>).quickCheckout === true);
    ...
    const taxonomyPills = [
      // Top-level(s) first, then the type — see categoryPills.
      ...categoryPills(p.categories).map((c) => `<a class="pill" href="/shop?category=${esc(c.slug)}">${esc(c.name)}</a>`),
    ...
    // Member pricing on the PDP (the grid already does this; the product page
    // did not). Opt-out per product via meta.noMemberDiscount so an at-cost
    ...
    html(reply, await page(`${p.name} — Therum Store`, `
      ...
      <div class="${pdpClasses}">
        ...
        <div class="pdp__info">
          ...
          ${quickCheckout
            ? `<div class="pdp-cta">
            <button class="btn btn--alt" id="add" ${firstAvailable && !hasChoice ? '' : 'disabled'}>${!firstAvailable ? 'Sold out' : 'Add to cart'}</button>
      ...
      <section class="pdp-reviews" data-reviews data-product-id="${p.id}">
        <h2 class="pdp-reviews__h">Reviews</h2>
        <div class="pdp-reviews__summary" data-reviews-summary></div>
        <div class="pdp-reviews__list" data-reviews-list></div>
        <details class="pdp-reviews__writewrap">
          <summary class="pdp-reviews__writebtn">Write a review</summary>
          <form class="pdp-reviews__form" data-review-form>
            <div class="pdp-reviews__stars" data-review-stars aria-label="Your rating"><button type="button" class="pdp-star" data-star="1" aria-label="1 star">&#9733;</button><button type="button" class="pdp-star" data-star="2" aria-label="2 stars">&#9733;</button><button type="button" class="pdp-star" data-star="3" aria-label="3 stars">&#9733;</button><button type="button" class="pdp-star" data-star="4" aria-label="4 stars">&#9733;</button><button type="button" class="pdp-star" data-star="5" aria-label="5 stars">&#9733;</button></div>
            <input type="hidden" name="rating" value="0">
            <input class="pdp-reviews__in" name="reviewerName" placeholder="Your name" maxlength="120" required>
            <input class="pdp-reviews__in" name="reviewerEmail" type="email" placeholder="Email (kept private)" required>
            <input class="pdp-reviews__in" name="title" placeholder="Title (optional)" maxlength="200">
            <textarea class="pdp-reviews__in pdp-reviews__body" name="body" placeholder="How was it?" maxlength="5000" required></textarea>
            <button class="btn" type="submit">Submit review</button>
            <p class="pdp-reviews__msg" data-review-msg></p>
          </form>
        </details>
      </section>`, `
// ── Reviews: list + rating summary + submit, wired to /shop/products/:id/reviews.
// The scheduled post-delivery "Write a review" email links here; before this the
// page had no review UI, so that email dead-ended and reviews could never be
// created (card star ratings stayed empty forever).
(function(){
  var box = document.querySelector('[data-reviews]');
  if (!box) return;
  var pid = box.getAttribute('data-product-id');
  var sumEl = box.querySelector('[data-reviews-summary]');
  var listEl = box.querySelector('[data-reviews-list]');
  var form = box.querySelector('[data-review-form]');
  var msg = box.querySelector('[data-review-msg]');
  function resc(s){ return String(s==null?'':s).replace(/[&<>"']/g,function(c){return ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'})[c];}); }
  function rstars(n){ n=Math.round(n||0); var o=''; for(var i=1;i<=5;i++){ o+='<span class="pdp-rstar'+(i<=n?' on':'')+'">&#9733;</span>'; } return o; }
  function rwhen(d){ try { return new Date(d).toLocaleDateString('en-US',{year:'numeric',month:'short',day:'numeric'}); } catch(e){ return ''; } }
  async function loadReviews(){
    try {
      var res = await fetch('/api/shop/products/'+encodeURIComponent(pid)+'/reviews');
      var data = await res.json();
      var s = data.summary || { average:0, count:0 };
      sumEl.innerHTML = s.count > 0
        ? '<span class="pdp-reviews__avg">'+rstars(s.average)+'</span> <strong>'+s.average+'</strong> <span class="pdp-reviews__cnt">'+s.count+' review'+(s.count===1?'':'s')+'</span>'
        : '<span class="pdp-reviews__none">No reviews yet \\u2014 be the first.</span>';
      var reviews = data.reviews || [];
      listEl.innerHTML = reviews.map(function(r){
        return '<div class="pdp-review"><div class="pdp-review__top"><span class="pdp-review__rs">'+rstars(r.rating)+'</span>'
          + (r.verified ? '<span class="pdp-review__verified">Verified buyer</span>' : '')
          + '<span class="pdp-review__by">'+resc(r.reviewerName)+' &middot; '+rwhen(r.createdAt)+'</span></div>'
          + (r.title ? '<div class="pdp-review__title">'+resc(r.title)+'</div>' : '')
          + '<div class="pdp-review__body">'+resc(r.body)+'</div></div>';
      }).join('');
    } catch(e){ /* leave the section quiet on failure */ }
  }
  var rating = 0;
  box.querySelectorAll('.pdp-star').forEach(function(btn){
    btn.addEventListener('click', function(){
      rating = parseInt(btn.getAttribute('data-star'),10) || 0;
      var hid = form.querySelector('[name=rating]'); if (hid) hid.value = String(rating);
      box.querySelectorAll('.pdp-star').forEach(function(b){ b.classList.toggle('on', (parseInt(b.getAttribute('data-star'),10)||0) <= rating); });
    });
  });
  if (form) form.addEventListener('submit', async function(e){
    ...
    if (!payload.rating) { msg.textContent = 'Please pick a star rating.'; return; }
    var submit = form.querySelector('[type=submit]'); if (submit) submit.disabled = true;
    msg.textContent = 'Submitting\\u2026';
    try {
      var pres = await fetch('/api/shop/products/'+encodeURIComponent(pid)+'/reviews', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) });
      if (!pres.ok) { var er = await pres.json().catch(function(){return {};}); throw new Error(er.message || 'Could not submit your review.'); }
      msg.textContent = 'Thanks! Your review will appear once approved.';
      form.reset(); rating = 0;
      box.querySelectorAll('.pdp-star').forEach(function(b){ b.classList.remove('on'); });
...
var MEMBER_PCT=${pdpMemberPct};function shown(c){return MEMBER_PCT>0?Math.round(c*(1-MEMBER_PCT/100)):c;}
// Warm the image cache up front: each colourway's photo (often an external POD
// CDN) is fetched on load, so choosing a swatch swaps instantly instead of
// waiting on a network round-trip after the click.
(function(){var seen={};VARIANTS.forEach(function(v){[].concat(v.image?[v.image]:[],(v.images||[]).map(function(g){return g.url;})).forEach(function(u){if(u&&!seen[u]){seen[u]=1;var im=new Image();im.src=u;}});});})();
...
function syncBuy(){
  ...
  var co=document.getElementById('checkout');if(co)co.disabled=!ok;
}
function applyVariant(v){
  sel=v;
  const fmt=(shown(v.price)/100).toLocaleString('en-US',{style:'currency',currency:'USD'});
...
if(sw)sw.addEventListener('click',(e)=>{
  ...
  // A swatch changes the picture immediately, before a size is picked (Woo
  // parity — the shopper expects the colour they tapped to show now). The
  // variant chosen for BUYING is still resolved by resolve() once size is set.
  var pv=VARIANTS.find(function(v){return v.color===pickColor&&v.image;})||VARIANTS.find(function(v){return v.color===pickColor;});
  if(pv)showVariantShots(pv);
...
if(picker)picker.addEventListener('click',(e)=>{
  ...
  document.getElementById('price').textContent=(shown(sel.price)/100).toLocaleString('en-US',{style:'currency',currency:'USD'});
...
function renderStrip(shots){
  ...
  // Mutate the existing <img> src rather than rebuilding innerHTML: the image
...
var _co=document.getElementById('checkout');
if(_co)_co.addEventListener('click',async function(e){if(!sel)return;var b=e.currentTarget;b.disabled=true;b.textContent='…';try{await addToCart(sel.id,qty);window.location.href='/checkout';}catch(err){b.disabled=false;b.textContent='Checkout';}});
`, {
      description: (p.description ?? '').replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 300)
        || `${p.name} from ${process.env.SITE_NAME || ''}.`,
  ...
  app.get('/careers/:slug', async (req, reply) => {
    ...
    if (!role) return reply.status(404).send(await page(`Role not found — ${process.env.SITE_NAME || ''}`,
    ...
    html(reply, await page(`${role.title} — Careers — ${process.env.SITE_NAME || ''}`,
      `<style>${CAREERS_CSS}</style>${rolePage(role)}`,
      CAREERS_RUNTIME,
      {
        description: `${role.title} at ${process.env.SITE_NAME || ''}. ${role.terms}. ${role.blurb}`,
  ...
  app.get('/order-tracking', async (req, reply) => {
    ...
    html(reply, await page(
      ...
      ORDER_TRACKING_RUNTIME,
      {
        ...
        description: 'Track your order — enter your order number and the email on the order to see carrier and delivery status.',
