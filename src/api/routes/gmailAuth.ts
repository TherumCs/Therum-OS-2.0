...
function originOf(req: FastifyRequest): string {
  // Mirror adminGoogleAuth's publicOrigin exactly: split a comma list and fall
  // back to https, never req.protocol — behind nginx the app listens on plain
  // http, so a missing X-Forwarded-Proto produced an http:// redirect_uri and
  // Google answered redirect_uri_mismatch while sign-in (https fallback) worked.
