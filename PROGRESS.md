...
- **Providers** carry `distribution`: `native` (built-in 2.0 engine), `ecosystem` (**Counter, Nexus = standalone own-versioned Therum products / spin-offs — dual function**), `platform` (WordPress), `custom` (per-site builds, e.g. `counter-sidemoney`).
- **Verified the Sidemoney composition live:** foundation=bricks, Commerce ON → `counter-sidemoney` [custom], Connections ON → `nexus` [ecosystem] = "Therum OS + Bricks + Nexus + custom Counter". Planned-provider select → 422.
...
- 1.9.44 (WordPress) is SUNSET; do not deploy the parked mu-plugin wiring. Ignore Sidemoney/bamleon.
