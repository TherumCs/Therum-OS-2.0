...
a compat surface. Sidemoney's local build (new session) is the first consumer.
...
- [ ] Inventory the actual WP functions the target themes call (grep a few real
      themes incl. whatever Sidemoney wants) — sizes the T1 shim honestly.
- [x] Bricks data format deep-dive (bricks_page_content meta JSON schema).
      ...
      bam-leon's real DB: Sidemoney page = 501 elements (text-basic 225,
...
Live E2E: real 501-element Sidemoney Bricks layout from bam-leon DB imported
→ 502-node canvas → published → renders on Base Theme
(/sidemoney-bricks-import-e2e); export round-trips all 501 elements with
...
imports. Live-verified: 18/18 Sidemoney assets pulled from bam-leon
