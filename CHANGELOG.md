...
and everything the real catalogue exposed got fixed. First real customer order
landed and confirmed the money path end-to-end. Then the money moved to where it
belongs: card, wallet and BNPL now settle through the reconnected **WooPayments**
merchant account, and the payments backend was rebuilt around both rails.

### Payments — routed through WooPayments
- **Card / wallets / BNPL now settle to the WooPayments account**
  (the configured merchant account) — the instant-payout-eligible merchant
  account where the payout bank lives — instead of the standalone Stripe account they had been
  hitting. Confirmed by a real controlled charge that landed on the right account
  and settled end-to-end. Reversible behind a single `cardProvider` setting
  (default keeps Stripe-direct; flip to `woopay`, flip back for instant rollback,
  zero code change). PayPal / Venmo stay on their own rail, untouched.
- **Headless charge path.** A carved mu-plugin route on the sealed WooPayments
  engine mints an unconfirmed intent (`Create_Intention`) and hands back its
  client_secret + the connected-account publishable key; the storefront confirms
  it in-page with Stripe.js against the account — no redirect, no off_session.
  Both the main checkout and the shop-grid buy box run this rail; a `cardProvider`
  flag routes card/wallet/BNPL to it with a byte-identical Stripe-direct fallback.
- **Settlement is client-driven, webhook-backed.** After the in-page confirm the
  order finalises immediately (poll the succeeded intent → `markPaid` → receipt /
  owner email / fulfillment); the engine's `woocommerce_payment_complete` → signed
  bridge webhook is the idempotent backup. (Fixed a gap where relying on the
  webhook alone left a genuinely-paid order stuck pending.)
- Certified both prerequisites from live plugin source before writing any charge
  code: the WooPayments account really is a different Stripe account than the
  direct key (so this moves real money), and the plugin's intent class returns a
  usable client_secret headlessly.

### Payments admin — unified redesign
- **Counter › Payments rebuilt** around the two-account reality (WooPayments +
  direct Stripe, both settling to one bank): a unified balance hero (total +
  per-account split), two symmetric account tiles (status rows + real action
  buttons for instant payout / deposit schedule / methods), and one **merged
  activity** ledger — payouts AND transactions across BOTH accounts, account-
  labelled, with real customer names and an account filter.
- Every WooPayments control surfaced in the backend over a read/write bridge
  (balance, payouts incl. instant, transactions, disputes, deposit schedule,
  method toggles) — no WordPress admin, no external links.

### Admin & storefront polish
- **Admin session no longer drops you.** Lifetime raised 12h → 30-day sliding, and
  the renewal cookie now carries the same `Domain` as login/logout, so a renewed
  session can't split into a stale host-only duplicate that reads as logged-out.
- **Account page tightened** — page title dropped, tabs grouped left instead of
  stretched edge-to-edge, empty-state overview cards fill the row.
- Checkout success screen now centres on desktop (was anchored in the old form
  column, reading as a stray mobile strip).

### Clustering & merchandising
- **Cluster primary no longer defaults onto a draft.** A group's primary
  resolved to the *earliest-created* member when no explicit override was set —
  so an older draft (not in any category) silently became the visible product
  and the whole cluster vanished from the storefront. Clusters now set an
  explicit primary. An example clustered product (three pushes) is one clustered
  card; two same-colour members that used to collapse to one swatch are now
  distinct **split-gradient swatches** (2 hex codes → a hard 135° split, e.g.
  Black/White vs Black/Green).
- **Per-colour variant images bound.** `ProductVariant.image` was NULL, so a
  swatch changed nothing. Colours are now mapped to their own flat shot (the
  storefront swatch/PDP both read `v.image`), verified against the DB colour name.
- **Swatch previews on click.** The PDP only swapped the picture once *both*
  colour and size were chosen; tapping a colour now shows that colour's photo
  immediately (WooCommerce parity), while the buy variant still resolves on size.
- **Sizes sort smallest→largest** everywhere (`sortSizes`) — a plain `.sort()`
  had been printing L, M, S, XL on cards, filters and the PDP.
- Vendor spec-sheet HTML stripped from descriptions; on-brand copy applied
  (carried from the reference store where it fit, newly written where it didn't).
  Duplicate vendor re-pushes removed; retail prices set.

### Storefront
- **Category hero logo centering** fixed — the shared fade-in keyframe ended at
  `transform:none` with `fill-mode:both`, which wiped the `translate(-50%,-50%)`
  and dropped the logo to a top-left anchor. Dedicated keyframe that keeps the
  translate at both ends.
- **Front-end pagination** now renders on collection (T2) pages too, and the
  default page size breaks the catalogue into pages instead of one long scroll.
- Homepage: two-column video band above Reserve Notes, seasonal product panels
  with a countdown, editorial shots restored.

### Checkout & commerce
- **Local pickup** — per-product eligibility (a subset of products), two
  named slots with dates, name/note surfaced at checkout.
- Optional **phone field** on the card quick-pay and the main checkout.
- Order-confirmation email delivery verified live (SMTP).

### Admin
- Products list: **newest-first** default sort + numbered pagination.
- **Sidebar highlight** setting (Quick Controls → Appearance): straight highlight
  by default, plus Band / Indicator-line / Underline modes.

### SEO & integrations
- Homepage `og:url` + canonical corrected (pointed at a stale homepage slug; now the
  root) and an absolute `og:image` added, so shared links render a real card.
- **Meta domain-verification** tag in `<head>` — the WordPress-era verification
  didn't survive the server move; re-verified for Instagram Shopping.
- **WooCompat** brought to byte-for-byte WooCommerce parity (diffed against a
  real store): JSON connect callback, `/wc/v3/data/currencies/current`, populated
  discovery `url`/`home`, attribute id-matching for POD providers, WP media
  endpoints. Tapstitch/PODpluser connect and sync end-to-end.

## [2.0.0-beta.8] — Storefront launch hardening (2026-08-09)

Shipping, wallets end-to-end, and the pre-launch audit pass.

- **Shipping system:** customers choose a rate at checkout; hybrid vendor/zone
  rates; admin grid to configure. Wired into both the main checkout and the
  quick-buy sheet.
- **Wallets that actually charge** in the main checkout and the buy box
  (Apple Pay / Google Pay / Link). The native sheet authorises first, then the
  order is written and charged inside its callback — a dismissed sheet leaves no
  unpaid order. Both flows are synchronous at the tap (iOS gesture law) with a
  timeout watchdog so the sheet can never hang on "Processing…".
- **Mobile checkout fixes:** payment methods tabbed and scrollable; pay-later
  and wallet pills full-width; the success/processing screen is hidden until an
  order exists and shows the order number; the summary bar reparents cleanly
  across the mobile boundary; footer dropped from checkout; horizontal overflow
  removed. Circular wallet discs (fixed dimensions, not `aspect-ratio`).
- **Admin:** five dead wires fixed (media delete, command-palette content
  search, Product Studio look panel, Connections prefill, assign-products
  roster); delete confirmations added.
- **Reliability:** nightly DB backup fixed (had failed 8 days on a 1 MB buffer);
  API memory cap raised; Redis eviction policy set to `noeviction` for BullMQ;
  worker shutdown closes the backup worker; rate limiter made atomic (no more
  stranded-counter lockouts).
- **Security (Forge audit):** neutered a fallback admin token that granted full
  API access on any gate miss; masked credential-bearing query params in request
  logs; patched two high-severity dependencies.
- **Tooling:** GitHub Actions CI (typecheck, build, storefront runtime gate,
  dependency audit, all three apps' type checks); `npm run test:runtime` gate
  for the embedded browser JS; README and architecture docs rewritten.
...
a real site, VPS deploy unproven. First consumer: the reference store build.


## [2.0.0-beta.1] — Pre-launch punch list (2026-07-25)

The alpha→beta gap-closers so the reference store build session starts clean.
...
before code. First consumer: the reference store local build session.
...
- 7/7 site tests: landing + homepage modes, SEO head injection, draft leak-proofing,
  ...
  landing with a real published case study, /work/<slug> rendering with correct
  canonical. **Full regression 116/116, exit 0.**
...
- **`UpdateContentInput = CreateContentInput.partial()` kept the .default()s** — any
  ...
  path un-typed AND un-published a real published case study and reset its
...
- Proof it rides the real pipeline: on first render the page surfaced a
  pre-existing case_study row invisible to the UI until today.
